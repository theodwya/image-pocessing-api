...
async def submit_job(job: JobRequest, background_tasks: BackgroundTasks):
    ...
    Returns:
        dict: Status message.
    """
    background_tasks.add_task(process_image, job.input_image, job.output_image, job.operation, job.priority)
    return {"status": "Job submitted successfully"}

@app.post("/submit_batch/")
async def submit_batch(batch_job: BatchJobRequest, background_tasks: BackgroundTasks):
    """
    ...
    Returns:
        dict: Status message.
    """
    jobs = [{"input_image": job.input_image, "output_image": job.output_image, "operation": job.operation} for job in batch_job.jobs]
